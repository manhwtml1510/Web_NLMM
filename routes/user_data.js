...
router.get('/gio-hang', async (req, res) => {
    let [rows] = await pool.query('SELECT gio_hang.id_san_pham,san_pham.ten_san_pham ,gio_hang.so_luong_san_pham FROM `gio_hang` JOIN san_pham ON gio_hang.id_san_pham = san_pham.id_san_pham WHERE `id_tai_khoan` = ?',
